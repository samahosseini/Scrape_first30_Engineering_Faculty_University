...
def scrape_data(url):
    ...
    name = name_div.find('h1', class_='page-title').span.text.strip() if name_div else 'Not found'
    
    # Extract the position
    positions = []
    position_div = soup.find('div', class_='field--name-field-person-official-job-title')
    if position_div:
        positions.append(position_div.find('div', class_='field__item').text.strip())
    position = positions[0] if positions else 'Not found'
    ...
    research_focus = ', '.join(paragraph.text.strip() for paragraph in research_div.find_all('li')) if research_div else 'Not found'

    # Extract the department names
    school_elem = soup.select_one('.block-field-blocknodecoc-personfield-person-school .field__item a')
    center_elem = soup.select_one('.block-field-blocknodecoc-personfield-person-center .field__item a')
...
csv_file = r'D:\Files\Upwork\Scrape\Us_30_Uni_engineering\Src\3_GeorgiaTech\CivilEnvironmentalEngineering\profiles_urls.csv'
...
output_csv_file = r'D:\Files\Upwork\Scrape\Us_30_Uni_engineering\Result\3_GeorgiaTech\GeorgiaTech_CivilEngineering.csv'
